...
type Service struct {
	...
	Ctx                context.Context
	Wg                 *sync.WaitGroup
	Relay              nostr2.RelayOrchestrator
	Cfg                *Config
...
func NewService(ctx context.Context) (*Service, error) {
	...
	relayOrchestrator := nostr2.NewRelayOrchestrator(ctx, nostr2.RelayConfig{
		Headers:     headers,
		MaximumIdle: 600 * time.Second,
	})
...
func (svc *Service) InfoHandler(c echo.Context) error {
	...
	filter := nostr2.Filter{
		Authors: []string{requestData.WalletPubkey},
		Kinds:   []int{NIP_47_INFO_EVENT_KIND},
		Limit:   1,
	}
	sub, err := svc.Relay.Subscribe(relayURL, []nostr2.Filter{filter})
	if err != nil {
...
func (svc *Service) PublishHandler(c echo.Context) error {
	...
	err := svc.Relay.Publish(ctx, relayURL, *requestData.SignedEvent)
	if err != nil {
		svc.Logger.WithError(err).WithFields(logrus.Fields{
			"event_id":  requestData.SignedEvent.ID,
			"relay_url": requestData.RelayUrl,
...
func (svc *Service) prepareNIP47Subscription(relayUrl, walletPubkey, webhookUrl string, requestEvent *RequestEvent) Subscription {
	return Subscription{
		...
		Limit:        1,
		RequestEvent: requestEvent,
		EventChan:    make(chan *nostr2.Event, 1),
...
func (svc *Service) startSubscription(ctx context.Context, subscription *Subscription, onReceiveEOS OnReceiveEOSFunc, handleEvent HandleEventFunc) {
	...
	for {
		...
		relaySubscription, err := svc.Relay.Subscribe(relayURL, []nostr2.Filter{*filter})
		if err != nil {
			svc.Logger.WithError(err).WithFields(logrus.Fields{
				"request_event_id": requestEventId,
				"subscription_id":  subscription.Uuid,
				"relay_url":        relayURL,
			}).Error("Failed to subscribe, retrying...")
...
func (svc *Service) publishRequestEvent(ctx context.Context, subscription *Subscription) {
	...
	err := svc.Relay.Publish(ctx, relayURL, *subscription.RequestEvent.SignedEvent)
	if err != nil {
...
func (svc *Service) handleResponseEvent(event *nostr2.Event, subscription *Subscription) {
	walletPubkey, clientPubkey := getPubkeys(subscription)
...
func (svc *Service) handleSubscribedEvent(event *nostr2.Event, subscription *Subscription) {
	svc.Logger.WithFields(logrus.Fields{
		"response_event_id":   event.ID,
...
func (svc *Service) postEventToWebhook(event *nostr2.Event, subscription *Subscription) {
...
func (svc *Service) subscriptionToFilter(subscription *Subscription) *nostr2.Filter {
	filter := nostr2.Filter{
